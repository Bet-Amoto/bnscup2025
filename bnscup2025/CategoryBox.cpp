...
void CategoryBox::draw(const Array<Die>& dices) const
{
	...
	m_scoreRect.rounded(5).draw(m_category->score ? ColorF{ 0.7 } : ColorF{ 1.0 }).drawFrame(3, ColorF{ 0.1 });
	FontAsset(U"Category")(m_category->name).draw(28, m_boxRect.x + 5, m_boxRect.y + 11, ColorF{ 0.1 });
	if (m_category->score.has_value())
	{
		FontAsset(U"Category")(U"{}"_fmt(m_category->score.value())).drawAt(m_scoreRect.center(), ColorF{ 0.1 });
	}
	else {
